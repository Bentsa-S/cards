...
import { MainControllerGame } from '../../../games/durack/main';
import './GamePageDurack.scss';

export const GamePageDurack = () => {
  const [loading, setLoading] = useState(true);
  const backgroundOption = Number(localStorage.getItem('backgroundOption')) || 1;
  const theme = localStorage.getItem('theme') || 'light';
  const canvasRef = useRef(null);
  const { idRoom, numberPlayer } = useParams();
  ...
  useEffect(() => {
    const initGame = async () => {
        ...
        const main = new MainControllerGame(pixiApp, numberPlayer, idRoom, 'vasa', 1);
